...
class Get_gradient_1(nn.Module):
    def __init__(self):
        super(Get_gradient_1, self).__init__()
        kernel_v = [[0, -1, 0],
                    [0, 0, 0],
                    [0, 1, 0]]
        kernel_h = [[0, 0, 0],
...
class EDGSP_SCTransNet(nn.Module):
    def __init__(self, config, n_channels=1, n_classes=1, img_size=256, vis=False, mode='train', deepsuper=True):
        ...
        self.outc = nn.Conv2d(in_channels + 1, n_classes, kernel_size=(1, 1), stride=(1, 1))
    ...
    def forward(self, x):
        ...
        #                  First Embedding
        # *************************************************
        a, b = torch.chunk(x, 2, dim=1)  # 64 224 224
        sob = self.grad(a)
        x = torch.cat([a, sob, b], dim=1)
        ...
        #                  Second Embedding
        # *************************************************
        out = self.outc(torch.cat([self.up_decoder1(d2, x1), b], dim=1))
        # deep supervision
        if self.deepsuper:
...
if __name__ == '__main__':
    ...
    model = EDGSP_SCTransNet(config_vit, n_channels=3, mode='train', deepsuper=True).cuda()
    inputs = torch.rand(1, 2, 256, 256).cuda()
    output = model(inputs)
