...
def random_crop_Pce(img, mask, mask_Pce, patch_size, pos_prob=None):
    h, w = img.shape
    if min(h, w) < patch_size:
        img = np.pad(img, ((0, max(h, patch_size) - h), (0, max(w, patch_size) - w)),
                     mode='constant')
        mask = np.pad(mask, ((0, max(h, patch_size) - h), (0, max(w, patch_size) - w)),
                      mode='constant')
        mask_Pce = np.pad(mask_Pce, ((0, max(h, patch_size) - h), (0, max(w, patch_size) - w)),
                      mode='constant')
        h, w = img.shape
    ...
    while 1:
        ...
        img_patch = img[h_start:h_end, w_start:w_end]
        mask_patch = mask[h_start:h_end, w_start:w_end]
        mask_Pce_patch = mask_Pce[h_start:h_end, w_start:w_end]
    ...
    return img_patch, mask_patch, mask_Pce_patch
